...
with service ID `myapplication3`
hosted in `us-east4`.
...
is the query named "GetGoalsForUser",
which can be accessed via the [ExampleConnector.getGoalsForUser] property.


An example of the property for a mutation
is the mutation named "CreateSubject",
which can be accessed via the [ExampleConnector.createSubject] property.
...
For example, the "GetGoalsForUser" query has 1 required variable ("userId")
and can be executed via the [ExampleConnector.getGoalsForUser]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.getGoalsForUser.execute(userId=java.util.UUID.randomUUID())
println("GetGoalsForUser query returned: ${queryResult.data}")
```
...
For example, the "CreateSubject" mutation has 2 required variables ("name" and "userId")
and can be executed via the [ExampleConnector.createSubject]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.createSubject.execute(name="waldo", userId=java.util.UUID.randomUUID())
println("CreateSubject mutation returned: ${mutationResult.data}")
```
...
For example, the "CreateSubject" mutation has 1 optional variable ("description")
and can be executed via the [ExampleConnector.createSubject]
property as follows:
